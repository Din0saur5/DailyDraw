...
describe('Edge function core handlers', () => {
  ...
  it('rejects upload over 10MB', async () => {
    ...
    await expect(
      handleUploadSign(
        { signer, currentUserId: 'user1', now: () => new Date('2024-01-01T00:00:00Z'), uuid: () => 'abc' },
  ...
  it('requires positive dimensions when creating submission', async () => {
    ...
    await expect(
      handleSubmissionUpsert(
        ...
        { dailyPromptId: '00000000-0000-4000-8000-000000000000', key: 'orig', width: 0, height: 0 },
      ),
    ).rejects.toBeInstanceOf(HttpError);
  });
  ...
  it('blocks removed submissions for non-owners when signing GET', async () => {
    ...
    await expect(
      handleImageSignGet(
        { submissionsRepo, signer, currentUserId: 'user1' },
        'orig',
      ),
    ).rejects.toBeInstanceOf(HttpError);
  });
  ...
  it('sorts prompts by difficulty order', async () => {
    const promptsRepo = {
      findWithinRange: jest.fn().mockResolvedValue([
        { id: '3', prompt_bank_id: 'a', prompt_text: 'medium', prompt_date: '2024-01-01', difficulty: 'medium' },
        { id: '1', prompt_bank_id: 'a', prompt_text: 'easy', prompt_date: '2024-01-01', difficulty: 'easy' },
      ]),
    };

    const result = await handlePromptsToday({ promptsRepo, now: () => new Date('2024-01-01T12:00:00Z') });
